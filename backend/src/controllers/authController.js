...
const registerUser = async (req, res, next) => {
  try {
    ...
    res.status(201).json({
      message:
        "Verification email sent! Please check your inbox and enter the code to complete registration.",
      user: {
        id: user.id,
        email: user.email,
...
const loginUser = async (req, res, next) => {
  try {
    ...
    const { user, accessToken, refreshToken, refreshTokenExpiry } =
...
const refreshAccessToken = async (req, res, next) => {
  try {
    const { accessToken, refreshToken, refreshTokenExpiry } =
      await authService.rotateRefreshToken(req.cookies.refreshToken);
